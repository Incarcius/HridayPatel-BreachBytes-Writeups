...
For a single qubit, the probability of measuring \(|1\rangle\) after a rotation by angle \(theta\) around the Y‑axis is:
...
- Phase rotations (`rz`) only change the **global phase** of the state and do **not** affect measurement probabilities in the Z‑basis. Padding, noise, and entropy terms in the circuit are red herrings for probability — ignore them when inverting the signal.  
- The values you recover from probabilities in `recovered_candidates.py` are of the form \(a ^ b\), where \(a\) and \(b\) are adjacent flag characters. Once you know one character (e.g., the leading `D` from `DJSISACA{`), you can walk the XOR chain to recover the rest.
